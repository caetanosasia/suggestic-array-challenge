...
docker exec myproject_api /bin/sh -c "pipenv run ./manage.py test --settings=myproject.myproject.settings.tests $TESTS_TO_RUN"

# You could add further commands on to the end of that command, eg:
# docker exec myproject_api /bin/sh -c "pipenv run ./manage.py test --settings=myproject.settings.tests $TESTS_TO_RUN ; pipenv run flake8"